...
db_foreach assignments {
    select distinct item_project_member_id as user_id,
        im_name_from_id(item_project_member_id) as username, 
        item_value as availability, 
        to_char(item_date,'YYMM') as start_date_pretty
    from im_planning_items 
    where item_project_phase_id = :project_id 
    and item_date >= date_trunc('month', current_date)
    order by username,start_date_pretty
} {
